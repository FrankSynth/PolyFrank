...
template <typename T, size_t Size> class CircularBuffer {
  ...
  public:
    CircularBuffer() : m_head(m_buffer), m_end(m_buffer + Size), m_tail(m_end - 1), m_size(0) {}

    CircularBuffer(const CircularBuffer<T, Size> &that) : CircularBuffer() { *this = that; }
    ...
    template <typename U, size_type ThatSize> CircularBuffer &operator=(const CircularBuffer<U, ThatSize> &that) {
        clear();
        for (size_type i = that.size() - min(that.size(), capacity()); i < that.size(); i++)
            push_back(that[i]);
        return *this;
    }

    reference operator[](size_type offset) {
        return const_cast<reference>(const_cast<const CircularBuffer &>(*this)[offset]);
    }
    ...
    uint8_t push_back(const_reference value) {
        uint8_t ret = 0;
        if (full()) {
            pop_front();
            ret = 1;
    ...
    uint8_t push_front(const_reference value) {
        uint8_t ret = 0;
        if (full()) {
            pop_back();
            ret = 1;
