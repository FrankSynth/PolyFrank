...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef *hdma) {
    ...
    if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */
    {
        ...
        registerValue &=
            ((uint32_t) ~(DMA_SxCR_MBURST | DMA_SxCR_PBURST | DMA_SxCR_PL | DMA_SxCR_MSIZE | DMA_SxCR_PSIZE |
    ...
    else if (IS_BDMA_CHANNEL_INSTANCE(hdma->Instance) != 0U) /* BDMA instance(s) */
    {
        ...
        registerValue &= ((uint32_t) ~(BDMA_CCR_PL | BDMA_CCR_MSIZE | BDMA_CCR_PSIZE | BDMA_CCR_MINC | BDMA_CCR_PINC |
                                       BDMA_CCR_CIRC | BDMA_CCR_DIR | BDMA_CCR_MEM2MEM | BDMA_CCR_DBM | BDMA_CCR_CT));

        /* Prepare the DMA Channel configuration */
        registerValue |= DMA_TO_BDMA_DIRECTION(hdma->Init.Direction) |
                         DMA_TO_BDMA_PERIPHERAL_INC(hdma->Init.PeriphInc) | DMA_TO_BDMA_MEMORY_INC(hdma->Init.MemInc) |
                         DMA_TO_BDMA_PDATA_SIZE(hdma->Init.PeriphDataAlignment) |
                         DMA_TO_BDMA_MDATA_SIZE(hdma->Init.MemDataAlignment) | DMA_TO_BDMA_MODE(hdma->Init.Mode) |
        ...
        hdma->StreamIndex = (((uint32_t)((uint32_t *)hdma->Instance) - (uint32_t)BDMA_Channel0) /
...
HAL_StatusTypeDef HAL_DMA_Start_IT_Stream(DMA_HandleTypeDef *hdma, uint32_t SrcAddress, uint32_t DstAddress,
    ...
    if (HAL_DMA_STATE_READY == hdma->State) {
        ...
        /* Configure the source, destination address and the data length */
        DMA_SetConfig(hdma, SrcAddress, DstAddress, dataSize);
    ...
    while (((*isr_reg) & cpltlevel_mask) == 0U) {
        ...
        if (Timeout != HAL_MAX_DELAY) {
            if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) {
                ...
                /* No need to check return value: as in this case we will return HAL_ERROR with HAL_DMA_ERROR_TIMEOUT
                 * error code  */
...
void HAL_DMA_IRQHandler(DMA_HandleTypeDef *hdma) {
    ...
    else if (IS_BDMA_CHANNEL_INSTANCE(hdma->Instance) != 0U) /* BDMA instance(s) */
    {
        ...
        /* Half Transfer Complete Interrupt management ******************************/
        if (((tmpisr_bdma & (BDMA_FLAG_HT0 << (hdma->StreamIndex & 0x1FU))) != 0U) &&
        ...
        /* Transfer Complete Interrupt management ***********************************/
        else if (((tmpisr_bdma & (BDMA_FLAG_TC0 << (hdma->StreamIndex & 0x1FU))) != 0U) &&
        ...
        /* Transfer Error Interrupt management **************************************/
        else if (((tmpisr_bdma & (BDMA_FLAG_TE0 << (hdma->StreamIndex & 0x1FU))) != 0U) &&
...
HAL_StatusTypeDef HAL_DMA_RegisterCallback(DMA_HandleTypeDef *hdma, HAL_DMA_CallbackIDTypeDef CallbackID,
    ...
    if (HAL_DMA_STATE_READY == hdma->State) {
        switch (CallbackID) {
            case HAL_DMA_XFER_CPLT_CB_ID: hdma->XferCpltCallback = pCallback; break;

            case HAL_DMA_XFER_HALFCPLT_CB_ID: hdma->XferHalfCpltCallback = pCallback; break;

            case HAL_DMA_XFER_M1CPLT_CB_ID: hdma->XferM1CpltCallback = pCallback; break;

            case HAL_DMA_XFER_M1HALFCPLT_CB_ID: hdma->XferM1HalfCpltCallback = pCallback; break;

            case HAL_DMA_XFER_ERROR_CB_ID: hdma->XferErrorCallback = pCallback; break;

            case HAL_DMA_XFER_ABORT_CB_ID: hdma->XferAbortCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_DMA_UnRegisterCallback(DMA_HandleTypeDef *hdma, HAL_DMA_CallbackIDTypeDef CallbackID) {
    ...
    if (HAL_DMA_STATE_READY == hdma->State) {
        switch (CallbackID) {
            ...
            case HAL_DMA_XFER_HALFCPLT_CB_ID: hdma->XferHalfCpltCallback = NULL; break;

            case HAL_DMA_XFER_M1CPLT_CB_ID: hdma->XferM1CpltCallback = NULL; break;

            case HAL_DMA_XFER_M1HALFCPLT_CB_ID: hdma->XferM1HalfCpltCallback = NULL; break;
...
static void DMA_CalcDMAMUXChannelBaseAndMask(DMA_HandleTypeDef *hdma) {
    ...
    if (IS_BDMA_CHANNEL_DMAMUX_INSTANCE(hdma->Instance) != 0U) {
        ...
        hdma->DMAmuxChannel =
            (DMAMUX_Channel_TypeDef *)((uint32_t)(((uint32_t)DMAMUX2_Channel0) + (stream_number * 4U)));
        ...
        hdma->DMAmuxChannel =
            (DMAMUX_Channel_TypeDef *)((uint32_t)(((uint32_t)DMAMUX1_Channel0) + (stream_number * 4U)));
