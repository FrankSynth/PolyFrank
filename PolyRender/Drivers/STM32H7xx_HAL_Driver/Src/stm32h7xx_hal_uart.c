...
HAL_StatusTypeDef HAL_UART_RegisterCallback(UART_HandleTypeDef *huart, HAL_UART_CallbackIDTypeDef CallbackID,
    ...
    if (huart->gState == HAL_UART_STATE_READY) {
        switch (CallbackID) {
            case HAL_UART_TX_HALFCOMPLETE_CB_ID: huart->TxHalfCpltCallback = pCallback; break;

            case HAL_UART_TX_COMPLETE_CB_ID: huart->TxCpltCallback = pCallback; break;

            case HAL_UART_RX_HALFCOMPLETE_CB_ID: huart->RxHalfCpltCallback = pCallback; break;

            case HAL_UART_RX_COMPLETE_CB_ID: huart->RxCpltCallback = pCallback; break;

            case HAL_UART_ERROR_CB_ID: huart->ErrorCallback = pCallback; break;

            case HAL_UART_ABORT_COMPLETE_CB_ID: huart->AbortCpltCallback = pCallback; break;

            case HAL_UART_ABORT_TRANSMIT_COMPLETE_CB_ID: huart->AbortTransmitCpltCallback = pCallback; break;

            case HAL_UART_ABORT_RECEIVE_COMPLETE_CB_ID: huart->AbortReceiveCpltCallback = pCallback; break;

            case HAL_UART_WAKEUP_CB_ID: huart->WakeupCallback = pCallback; break;

            case HAL_UART_RX_FIFO_FULL_CB_ID: huart->RxFifoFullCallback = pCallback; break;

            case HAL_UART_TX_FIFO_EMPTY_CB_ID: huart->TxFifoEmptyCallback = pCallback; break;

            case HAL_UART_MSPINIT_CB_ID: huart->MspInitCallback = pCallback; break;

            case HAL_UART_MSPDEINIT_CB_ID: huart->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_UART_UnRegisterCallback(UART_HandleTypeDef *huart, HAL_UART_CallbackIDTypeDef CallbackID) {
    ...
    if (HAL_UART_STATE_READY == huart->gState) {
        ...
        switch (CallbackID) {
            case HAL_UART_MSPINIT_CB_ID: huart->MspInitCallback = HAL_UART_MspInit; break;

            case HAL_UART_MSPDEINIT_CB_ID: huart->MspDeInitCallback = HAL_UART_MspDeInit; break;
...
/////////////////////////////////////////////////////////////////////
/////////////////Custom DMA endless function/////////////////////////
/////////////////////////////////////////////////////////////////////
HAL_StatusTypeDef HAL_UART_Receive_Stream_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t dataSize) {
    /* Check that a Rx process is not already ongoing */
    if (huart->RxState == HAL_UART_STATE_READY) {
        if (pData == NULL) {
            return HAL_ERROR;
        ...
        return (UART_Start_Receive_Stream_DMA(huart, pData, dataSize));
    }
    else {
        return HAL_BUSY;
    }
...
HAL_StatusTypeDef UART_Start_Receive_Stream_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t dataSize) {
    ...
    if (huart->hdmarx != NULL) {
        ...
        if (HAL_DMA_Start_IT_Stream(huart->hdmarx, (uint32_t)&huart->Instance->RDR, (uint32_t)huart->pRxBuffPtr,
                                    dataSize) != HAL_OK) {
            /* Set error code to DMA */
            huart->ErrorCode = HAL_UART_ERROR_DMA;
...
HAL_StatusTypeDef UART_SetConfig(UART_HandleTypeDef *huart) {
    ...
    if (UART_INSTANCE_LOWPOWER(huart)) {
        ...
        switch (clocksource) {
            case UART_CLOCKSOURCE_D3PCLK1: pclk = HAL_RCCEx_GetD3PCLK1Freq(); break;
            ...
            case UART_CLOCKSOURCE_LSE: pclk = (uint32_t)LSE_VALUE; break;
            default:
                pclk = 0U;
                ret = HAL_ERROR;
                break;
    ...
    /* Check UART Over Sampling to set Baud Rate Register */
        ...
        switch (clocksource) {
            case UART_CLOCKSOURCE_D2PCLK1: pclk = HAL_RCC_GetPCLK1Freq(); break;
            case UART_CLOCKSOURCE_D2PCLK2: pclk = HAL_RCC_GetPCLK2Freq(); break;
            case UART_CLOCKSOURCE_PLL2:
                HAL_RCCEx_GetPLL2ClockFreq(&pll2_clocks);
                pclk = pll2_clocks.PLL2_Q_Frequency;
                break;
